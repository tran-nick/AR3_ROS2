...
void TeensyDriver::init(std::string port, int baudrate, int num_joints, std::vector<double>& enc_steps_per_deg)
{
    ...
    if (ec)
    {
        RCLCPP_WARN(rclcpp::get_logger("TeensyDriver"),ec.message());
        RCLCPP_WARN(rclcpp::get_logger("TeensyDriver"),"Failed to connect to serial port %s", port.c_str());
        return; 
    }
    else
    {
        ...
        //if this don't work, replace rclcpp::get_logger() with LOGGER arg
        RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"Successfully connected to serial port %s", port.c_str());
    }
    ...
    RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"Successfully initialised driver on port %s", port.c_str());
...
void TeensyDriver::emergencyStop()
{
    std::string outMsg = "ES\n";    // "Emergency Stop"
    sendCommand(outMsg);
}

void TeensyDriver::clearEStop()
{
    std::string outMsg = "RE\n";    //  "REsume"
    sendCommand(outMsg);
}
...
void TeensyDriver::exchange(std::string outMsg)
{
    ...
    if (header == "ST")
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"ST header");   // debug
        checkInit(inMsg);   
    }
    else if (header == "JC")
    {
        // encoder calibration values
        RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"JC header");   // debug
        updateEncoderCalibrations(inMsg);
    }
    else if (header == "JP")
    {
        // encoder steps
        RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"JP header");   // debug
        updateEncoderSteps(inMsg);
    }
    else if (header == "ES")
    {
        RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"Teesny driver entering STATE_ERR");   // debug
    }
    else if (header == "RE")
    {
        // emergency stop
        RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"Teesny driver returning from STATE_ERR");   // debug
    }
    else
    {
        // unknown header
        RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"Unknown header %s", header);
    } 
}
...
void TeensyDriver::checkInit(std::string msg)
{
    ...
    else
    {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("TeensyDriver"),"Firmware version mismatch %s", version);
    }  
}
...
void TeensyDriver::updateEncoderCalibrations(std::string msg)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("TeensyDriver"),"Successfully updated encoder calibrations");
}
